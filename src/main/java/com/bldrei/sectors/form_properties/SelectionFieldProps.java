...
public record SelectionFieldProps<T>(List<T> options) implements FormFieldProps {
  ...
  public static <V> SelectionFieldProps<HierarchicSelectionFieldTreeNode<V>> multilayer(List<HierarchicSelectionFieldTreeNode<V>> values) {
    return new SelectionFieldProps<>(values);
  }
}
